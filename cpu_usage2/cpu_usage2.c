...
void usage(char *argv[]) {
    ...
    printf(
        "-w %%\tSet warning (color orange) for cpu usage. (default: none)\n");
...
void display(const char *label, double used, int const warning,
             const char *warn_color, int const critical, const char *crit_color,
             int const decimals) {
    if (critical != 0 && used > critical) {
        printf("%s<span color='%s'>", label, crit_color);
    } else if (warning != 0 && used > warning) {
        printf("%s<span color='%s'>", label, warn_color);
    } else {
        printf("%s<span>", label);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (envvar) warning = atoi(envvar);
    envvar = getenv("CRIT_PERCENT");
    if (envvar) critical = atoi(envvar);
    envvar = getenv("DECIMALS");
    if (envvar) decimals = atoi(envvar);
    envvar = getenv("LABEL");
    if (envvar) label = envvar;
    envvar = getenv("CRIT_COLOR");
    if (envvar) crit_color = envvar;
    envvar = getenv("WARN_COLOR");
    if (envvar) warn_color = envvar;
    ...
    while (1) {
        ...
        display(label, 100.0D * (used - old_used) / (total - old_total),
                warning, warn_color, critical, crit_color, decimals);
        fflush(stdout);
        old_total = total;
        old_used = used;
    }
